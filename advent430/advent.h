...
#define AT(OBJ)                                                                \
  (game.objects[OBJ].place == game.loc || game.objects[OBJ].fixed == game.loc)
...
#define PCT(N) (glulx_random(100) < (N))
#define GSTONE(OBJ)                                                            \
...
struct game_t {
  int32_t
      lcg_x; // Unused now, but leave it here so we don't break the save format
