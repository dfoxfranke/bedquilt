...
static phase_codes_t attack(command_t command) {
  ...
  if (obj == VEND) {
    state_change(VEND, game.objects[VEND].prop == VEND_BLOCKS ? VEND_UNBLOCKS
                                                              : VEND_BLOCKS);
...
static phase_codes_t bigwords(vocab_t id) {
  ...
  int foobar = game.foobar < 0 ? -game.foobar : game.foobar;
...
static phase_codes_t vcarry(verb_t verb, obj_t obj) {
  ...
  if (obj == INTRANSITIVE) {
    ...
    if (game.locs[game.loc].atloc == NO_OBJECT ||
        game.link[game.locs[game.loc].atloc] != 0 || atdwrf(game.loc) > 0) {
      return GO_UNKNOWN;
    }
  ...
  if (game.objects[obj].fixed != IS_FREE) {
    ...
    case PLANT:
      ...
      rspeak(game.objects[PLANT].prop <= PLANT_THIRSTY ? DEEP_ROOTS
                                                       : YOU_JOKING);
      break;
    case BEAR:
      rspeak(game.objects[BEAR].prop == SITTING_BEAR ? BEAR_CHAINED
                                                     : YOU_JOKING);
      break;
    case CHAIN:
      rspeak(game.objects[BEAR].prop != UNTAMED_BEAR ? STILL_LOCKED
                                                     : YOU_JOKING);
      break;
    case RUG:
  ...
  if ((obj == BIRD || obj == CAGE) && (game.objects[BIRD].prop == BIRD_CAGED ||
...
static phase_codes_t find(verb_t verb, obj_t obj) {
  ...
  if (AT(obj) || (LIQUID() == obj && AT(BOTTLE)) || obj == LIQLOC(game.loc) ||
...
static phase_codes_t light(verb_t verb, obj_t obj) {
  ...
  case URN:
    state_change(URN,
                 game.objects[URN].prop == URN_EMPTY ? URN_EMPTY : URN_LIT);
    break;
  case LAMP:
    if (game.limit < 0) {
      rspeak(LAMP_OUT);
...
static phase_codes_t quit(void) {
   ...
   * wants. */
  if (yes_or_no(arbitrary_messages[REALLY_QUIT], arbitrary_messages[OK_MAN],
...
/*  Read.  Print stuff based on objtxt.  Oyster (?) is special case. */
{
  if (command.obj == INTRANSITIVE) {
    ...
    for (int i = 1; i <= NOBJECTS; i++) {
      if (HERE(i) && objects[i].texts[0] != NULL && !OBJECT_IS_STASHED(i)) {
...
static phase_codes_t throwit(command_t command) {
  ...
  if (command.obj != AXE) {
    ...
    if (glulx_random(NDWARVES + 1) < game.dflag) {
...
static phase_codes_t wave(verb_t verb, obj_t obj) {
  ...
  if (game.objects[BIRD].prop == BIRD_UNCAGED &&
    ...
    if (game.closed) {
      rspeak((game.objects[BIRD].prop == BIRD_CAGED) ? CAGE_FLY : FREE_FLY);
      return GO_DWARFWAKE;
    }
