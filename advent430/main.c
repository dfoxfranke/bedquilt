    ...
    for (int hint = 0; hint < NHINTS; hint++) {
      ...
      if (game.hints[hint].lc >= hints[hint].turns) {
        ...
        case 0:
          ...
          return;
        case 1: /* bird */
          if (game.objects[BIRD].place == game.loc && TOTING(ROD) &&
              game.oldobj == BIRD) {
            break;
          }
          return;
        ...
        case 3: /* maze */
          ...
          return;
        case 4: /* dark */
          if (!OBJECT_IS_NOTFOUND(EMERALD) && OBJECT_IS_NOTFOUND(PYRAMID)) {
            break;
          }
          game.hints[hint].lc = 0;
          return;
        ...
        case 8: /* ogre */
          ...
          return;
        case 9: /* jade */
          if (game.tally == 1 && OBJECT_IS_STASHED_OR_UNSEEN(JADE)) {
            break;
          }
          game.hints[hint].lc = 0;
          return;
        ...
        game.hints[hint].lc = 0;
        if (!yes_or_no(hints[hint].question, arbitrary_messages[NO_MESSAGE],
                       arbitrary_messages[OK_MAN])) {
          return;
        }
        rspeak(HINT_COST, hints[hint].penalty, hints[hint].penalty);
        game.hints[hint].used =
            yes_or_no(arbitrary_messages[WANT_HINT], hints[hint].hint,
                      arbitrary_messages[OK_MAN]);
...
static bool dwarfmove(void) {
  ...
  if (game.dflag == 1) {
    ...
    game.dflag = 2;
    for (int i = 1; i <= 2; i++) {
      int j = 1 + glulx_random(NDWARVES - 1);
      if (PCT(50)) {
  ...
  for (int i = 1; i <= NDWARVES; i++) {
    ...
    if (game.dwarves[i].oldloc == game.dwarves[i].loc) {
      ...
      if (glulx_random(1000) < 95 * (game.dflag - 2)) {
        ++stick;
      }
    }
  }
...
static void playermove(int motion) {
  ...
  if (motion == NUL) {
     ...
     */
    rspeak((OUTSID(game.loc) && game.loc != LOC_GRATE) ? FOLLOW_STREAM
                                                       : NEED_DETAIL);
    return;
  } else {
...
static void lampcheck(void) {
  ...
  if (game.limit <= WARNTIME) {
    if (HERE(BATTERY) && game.objects[BATTERY].prop == FRESH_BATTERIES &&
        HERE(LAMP)) {
...
static void listobjects(void) {
  ...
  if (!DARK(game.loc)) {
    ...
    for (int i = game.locs[game.loc].atloc; i != 0; i = game.link[i]) {
      ...
      if (obj == STEPS) {
        kk = (game.loc == game.objects[STEPS].fixed) ? STEPS_UP : STEPS_DOWN;
      }
  ...
  if (command->word[0].type == MOTION && command->word[0].id == ENTER &&
    ...
    if (command->word[0].type == OBJECT) {
      ...
      if (command->word[0].id == GRATE) {
        ...
        if (game.loc == LOC_START || game.loc == LOC_VALLEY ||
            game.loc == LOC_SLIT) {
      ...
      if (command->word[0].id == CAGE && command->word[1].id == BIRD &&
          HERE(CAGE) && HERE(BIRD)) {
  ...
  while (command.state != EXECUTED) {
    ...
    while (command.state <= GIVEN) {
      ...
      if (game.closed) {
        ...
        if ((OBJECT_IS_NOTFOUND(OYSTER) || OBJECT_IS_STASHED(OYSTER)) &&
            TOTING(OYSTER)) {
          pspeak(OYSTER, look, true, 1);
        }
        for (size_t i = 1; i <= NOBJECTS; i++) {
          if (TOTING(i) && (OBJECT_IS_NOTFOUND(i) || OBJECT_IS_STASHED(i))) {
            game.objects[i].prop = OBJECT_STASHED(i);
          }
        }
      }
      ...
      while (command.state == PREPROCESSED) {
        ...
        if (strncasecmp(command.word[0].raw, "go", sizeof("go")) == 0 &&
...
void glulx_main() {
  root_window = glk_window_open(NULL, 0, 0, wintype_TextBuffer, 0);
