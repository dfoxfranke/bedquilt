...
//!   [`derefl`], [`lloc`] or one of the reimports [`f32_to_imm`],
//!   [`f64_to_imm`], or [`load_local`].
//! * To construct a store operand: use [`push`], [`discard`], [`storel`],
//!   [`sloc`] or the reimport [`store_local`].
...
//!   avoid colliding with Rust keywords.
//! 
...
// Some of this module was generated by a Python script. The source to the
// script is in comments at the bottom of the module.

use crate::cast::CastSign;
...
use crate::{CallingConvention, Item};
...
pub fn fnhead_local<L>(nargs: u32) -> (Option<L>, Item<L>) {
    (None, Item::FnHeader(CallingConvention::ArgsInLocals, nargs))
}

/// Constructs a function header item with the `ArgsOnStack` calling convention.
pub fn fnhead_stack<L>(nargs: u32) -> (Option<L>, Item<L>) {
    (None, Item::FnHeader(CallingConvention::ArgsOnStack, nargs))
}
